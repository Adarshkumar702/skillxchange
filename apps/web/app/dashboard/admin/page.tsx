...
export default function AdminPage() {
  ...
  const deleteUserMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setTimeout(() => setRemoveSuccessMsg(''), 3500);
    },
  });
  ...
  const rawUsersList = usersRes?.data?.users || analyticsRes?.data?.recentUsers || [];
  const reports = reportsRes?.data || [];

  const usersList = rawUsersList.filter((u: any) => {
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-amber-950/30 border border-amber-500/20 space-y-2 shadow-lg">
              ...
              <p className="text-[11px] font-semibold text-slate-400">{overview?.dailyActiveUsers || 0} DAU • {overview?.monthlyActiveUsers || 0} MAU</p>
            </div>

            <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950/30 border border-indigo-500/20 space-y-2 shadow-lg">
              <div className="flex justify-between items-center text-slate-400 text-xs font-bold">
                <span>SKILLS DATABASE CATALOG</span>
                <BookOpen className="w-4 h-4 text-indigo-400" />
              </div>
              <p className="text-3xl font-black text-indigo-400">{overview?.totalSkills || 0}</p>
              <p className="text-[11px] font-semibold text-slate-400">Verified central skill topics</p>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-cyan-950/30 border border-cyan-500/20 space-y-2 shadow-lg">
              ...
              <p className="text-3xl font-black text-cyan-400">{overview?.totalSwaps || 0}</p>
              <p className="text-[11px] font-semibold text-slate-400">{overview?.completedSwaps || 0} completed swaps</p>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 via-slate-900 to-amber-950/30 border border-amber-500/20 space-y-2 shadow-lg">
              ...
              <p className="text-3xl font-black text-amber-400">{overview?.averageRating || 5.0} ★</p>
              <p className="text-[11px] font-semibold text-slate-400">{overview?.totalRatings || 0} peer ratings submitted</p>
            </div>
          </div>
